    ...
    public partial class Sign : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (result != null && "ok" == result.ToLower())
            {
                ...
                {
                    throw new ConfigurationErrorsException("Fejl - logonto parameter fra konfiguration mangler, eller er tomt");
                }
                var status = SignHandler.ValidateSignatureAgainstAgreement(loginData, textToBeSigned, signTextTransformation, Challenge(), logonto);

                if (!(status.Certificate is PocesCertificate || status.Certificate is MocesCertificate))
                {
                    Session.Add("errorText", "Det benyttede certifikat er ikke af korrekt type. Forventede personligt certifikat eller medarbejdercertifikat, fik " + ErrorHandler.CertificateType(status));
                }
